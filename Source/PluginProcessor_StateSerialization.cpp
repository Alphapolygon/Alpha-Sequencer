...
void MiniLAB3StepSequencerAudioProcessor::updateUiMetadataFromVar(const juce::var& stateVar)
{
    ...
    if (object->hasProperty("themeIdx"))
        themeIndex.store(static_cast<int>(object->getProperty("themeIdx")));

    if (object->hasProperty("visibleTracks"))
        visibleTracks.store(juce::jlimit(1, 16, static_cast<int>(object->getProperty("visibleTracks"))));

    if (object->hasProperty("footerTab"))
        footerTabIndex.store(ParameterNames::footerTabIndex(object->getProperty("footerTab").toString()));

    if (object->hasProperty("activeIdx"))
        activePatternIndex.store(juce::jlimit(0, MiniLAB3Seq::kNumPatterns - 1, static_cast<int>(object->getProperty("activeIdx"))));
    ...
    if (object->hasProperty("activeSection"))
        activeSection.store(juce::jlimit(-1, MiniLAB3Seq::kNumPages - 1, static_cast<int>(object->getProperty("activeSection"))));

    if (object->hasProperty("uiScale"))
        uiScale.store(static_cast<float>(static_cast<double>(object->getProperty("uiScale"))));
}
...
void MiniLAB3StepSequencerAudioProcessor::setStepDataFromVar(const juce::var& stateVar)
{
    ...
    auto parsePattern = [&](juce::DynamicObject* dataObj, PatternSnapshot& patternWrite, int pIdx) {
        ...
        {
            if (auto* randomAmountRows = dataObj->getProperty("randomAmounts").getArray())
            {
                for (int track = 0; track < juce::jmin(MiniLAB3Seq::kNumTracks, randomAmountRows->size()); ++track)
                {
                    if (auto* values = randomAmountRows->getReference(track).getArray())
                    {
                        for (int i = 0; i < juce::jmin(6, values->size()); ++i)
                            trackRandomAmounts[pIdx][track][i].store(readFloatVar(values->getReference(i)));
                    }
                }
            }
        }
        ...
        if (pIdx == activePatternIndex.load())
        {
            if (dataObj->hasProperty("midiKeys"))
            {
                if (auto* notes = dataObj->getProperty("midiKeys").getArray())
                {
                    for (int track = 0; track < juce::jmin(MiniLAB3Seq::kNumTracks, notes->size()); ++track)
                        setParameterFromPlainValue("note_" + juce::String(track + 1),
                                                   static_cast<float>(MiniLAB3Seq::parseMidiNoteName(notes->getReference(track).toString())));
                }
            }
            ...
            if (dataObj->hasProperty("trackStates"))
            {
                ...
                {
                    for (int track = 0; track < juce::jmin(MiniLAB3Seq::kNumTracks, states->size()); ++track)
                    {
                        if (auto* trackState = states->getReference(track).getDynamicObject())
                        {
                            setParameterFromPlainValue("mute_" + juce::String(track + 1),
                                                       readBoolVar(trackState->getProperty("mute")) ? 1.0f : 0.0f);
                            setParameterFromPlainValue("solo_" + juce::String(track + 1),
                                                       readBoolVar(trackState->getProperty("solo")) ? 1.0f : 0.0f);

                            if (trackState->hasProperty("scale"))
                                trackScales[track].store(static_cast<int>(trackState->getProperty("scale")));
                            if (trackState->hasProperty("length"))
                                trackLengths[pIdx][track].store(juce::jlimit(1, MiniLAB3Seq::kNumSteps, static_cast<int>(trackState->getProperty("length"))));
                            if (trackState->hasProperty("timeDivision"))
                                trackTimeDivisions[pIdx][track].store(juce::jlimit(0, 4, static_cast<int>(trackState->getProperty("timeDivision"))));
                            if (trackState->hasProperty("midiChannel"))
                                trackMidiChannels[track].store(juce::jlimit(1, 16, static_cast<int>(trackState->getProperty("midiChannel"))));
                        }
                    }
                }
            }
    ...
    if (object->hasProperty("patterns"))
    {
        modifySequencerState([&](MatrixSnapshot& writeMatrix) {
            if (auto* patternArray = object->getProperty("patterns").getArray())
            {
                for (int i = 0; i < juce::jmin(MiniLAB3Seq::kNumPatterns, patternArray->size()); ++i)
                {
                    if (auto* patternObject = patternArray->getReference(i).getDynamicObject())
                        parsePattern(patternObject->getProperty("data").getDynamicObject(), writeMatrix[i], i);
                }
            }
        });
    }
...
juce::var MiniLAB3StepSequencerAudioProcessor::buildPatternDataVar(int patternIndex) const
{
    ...
    for (int track = 0; track < MiniLAB3Seq::kNumTracks; ++track)
    {
        ...
        for (int step = 0; step < MiniLAB3Seq::kNumSteps; ++step)
        {
            ...
            aRow.add(stepData.isActive);
            vRow.add(juce::jlimit(0, 100, static_cast<int>(std::round(stepData.velocity * 100.0f))));
            pRow.add(juce::jlimit(0, 100, static_cast<int>(std::round(stepData.probability * 100.0f))));
            gRow.add(juce::jlimit(0, 100, static_cast<int>(std::round(stepData.gate * 100.0f))));
            shRow.add(juce::jlimit(0, 100, static_cast<int>(std::round(stepData.shift * 100.0f))));
            swRow.add(juce::jlimit(0, 100, static_cast<int>(std::round(stepData.swing * 100.0f))));
            rRow.add(stepData.repeats);
            piRow.add(stepData.pitch);
        }

        for (int i = 0; i < 6; ++i)
            randArr.add(trackRandomAmounts[patternIndex][track][i].load());

        activeSteps.add(aRow);
        velocities.add(vRow);
        probabilities.add(pRow);
        gates.add(gRow);
        shifts.add(shRow);
        swings.add(swRow);
        repeats.add(rRow);
        ...
        state->setProperty("midiChannel", trackMidiChannels[track].load());
        state->setProperty("scale", trackScales[track].load());
        state->setProperty("length", trackLengths[patternIndex][track].load());
        state->setProperty("timeDivision", trackTimeDivisions[patternIndex][track].load());
    ...
    data->setProperty("pitches", pitches);
...
juce::var MiniLAB3StepSequencerAudioProcessor::buildCurrentPatternStateVar() const
{
    ...
    const int patternIndex = activePatternIndex.load();
    root->setProperty("patternData", buildPatternDataVar(patternIndex));
    ...
    root->setProperty("activeSection", activeSection.load());
    root->setProperty("activePatternIndex", patternIndex);
...
juce::var MiniLAB3StepSequencerAudioProcessor::buildFullUiStateVarForEditor() const
{
    ...
    for (int i = 0; i < MiniLAB3Seq::kNumPatterns; ++i)
    {
        ...
        patternObject->setProperty("id", patternUUIDs[i]);
        patternObject->setProperty("name", "Pattern " + juce::String(MiniLAB3Seq::kPatternLabels[i]));
        patternObject->setProperty("data", buildPatternDataVar(i));
        patterns.add(juce::var(patternObject.get()));
    }

    root->setProperty("patterns", patterns);
    root->setProperty("activeIdx", activePatternIndex.load());
    root->setProperty("selectedTrack", currentInstrument.load());
    root->setProperty("currentPage", currentPage.load());
    root->setProperty("activeSection", activeSection.load());
    root->setProperty("themeIdx", themeIndex.load());
    root->setProperty("uiScale", uiScale.load());
    root->setProperty("visibleTracks", visibleTracks.load());
    root->setProperty("footerTab", ParameterNames::footerTabName(footerTabIndex.load()));
    return juce::var(root.get());
}
...
void MiniLAB3StepSequencerAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    ...
    for (int pattern = 0; pattern < MiniLAB3Seq::kNumPatterns; ++pattern)
    {
        ...
        for (int track = 0; track < MiniLAB3Seq::kNumTracks; ++track)
        {
            ...
            trackElement->setAttribute("length", trackLengths[pattern][track].load());
            trackElement->setAttribute("timeDivision", trackTimeDivisions[pattern][track].load());
            trackElement->setAttribute("midiChannel", trackMidiChannels[track].load());
            trackElement->setAttribute("scale", trackScales[track].load());

            juce::String steps, velocities, gates, probabilities, repeats, shifts, swings, pitches, randoms;
            for (int step = 0; step < MiniLAB3Seq::kNumSteps; ++step)
            {
                const auto& stepData = matrix[pattern][track][step];
                steps += stepData.isActive ? "1" : "0";
                velocities += juce::String(stepData.velocity) + ",";
                gates += juce::String(stepData.gate) + ",";
                probabilities += juce::String(stepData.probability) + ",";
                repeats += juce::String(stepData.repeats) + ",";
                shifts += juce::String(stepData.shift) + ",";
                swings += juce::String(stepData.swing) + ",";
                pitches += juce::String(stepData.pitch) + ",";
            }

            for (int i = 0; i < 6; ++i)
                randoms += juce::String(trackRandomAmounts[pattern][track][i].load()) + ",";
            ...
            trackElement->setAttribute("pitches", pitches);
            trackElement->setAttribute("randomAmounts", randoms);
        }
    }

    auto* uiXml = xml->createNewChildElement("UIState");
    uiXml->setAttribute("activePatternIndex", activePatternIndex.load());
    uiXml->setAttribute("currentInstrument", currentInstrument.load());
    uiXml->setAttribute("currentPage", currentPage.load());
    uiXml->setAttribute("activeSection", activeSection.load());
    uiXml->setAttribute("themeIdx", themeIndex.load());
    uiXml->setAttribute("footerTab", footerTabIndex.load());
    uiXml->setAttribute("uiScale", static_cast<double>(uiScale.load()));
    uiXml->setAttribute("visibleTracks", visibleTracks.load());
...
void MiniLAB3StepSequencerAudioProcessor::setStateInformation(const void* data, int size)
{
    ...
    if (xmlState->hasTagName(apvts.state.getType()))
    {
        ...
        for (auto* child = apvtsXml->getFirstChildElement(); child != nullptr;)
        {
            ...
            if (child->hasTagName("Patterns") || child->hasTagName("UIState"))
    ...
    modifySequencerState([&](MatrixSnapshot& writeMatrix) {
        if (auto* patternsXml = xmlState->getChildByName("Patterns"))
        {
            ...
            {
                const int patternIndex = patternXml->getIntAttribute("index", 0);
                if (patternIndex < 0 || patternIndex >= MiniLAB3Seq::kNumPatterns)
                    continue;

                int track = 0;
                for (auto* trackElement : patternXml->getChildIterator())
                {
                    if (!trackElement->hasTagName("Track") || track >= MiniLAB3Seq::kNumTracks)
                        continue;

                    trackMidiChannels[track].store(trackElement->getIntAttribute("midiChannel", 1));
                    if (trackElement->hasAttribute("scale"))
                        trackScales[track].store(trackElement->getIntAttribute("scale", 0));

                    trackLengths[patternIndex][track].store(juce::jlimit(1, MiniLAB3Seq::kNumSteps,
                        trackElement->getIntAttribute("length", 16)));
                    trackTimeDivisions[patternIndex][track].store(juce::jlimit(0, 4,
                        trackElement->getIntAttribute("timeDivision", 3)));

                    const auto steps = trackElement->getStringAttribute("steps");
                    juce::StringArray velocities, gates, probabilities, repeats, shifts, swings, pitches, randoms;
                    ...
                    pitches.addTokens(trackElement->getStringAttribute("pitches"), ",", "");
                    randoms.addTokens(trackElement->getStringAttribute("randomAmounts"), ",", "");

                    for (int step = 0; step < MiniLAB3Seq::kNumSteps; ++step)
                    {
                        auto& stepData = writeMatrix[patternIndex][track][step];
                        stepData.isActive = (step < steps.length() && steps[step] == '1');
                        if (step < velocities.size()) stepData.velocity = velocities[step].getFloatValue();
                        if (step < gates.size()) stepData.gate = gates[step].getFloatValue();
                        if (step < probabilities.size()) stepData.probability = probabilities[step].getFloatValue();
                        if (step < repeats.size()) stepData.repeats = repeats[step].getIntValue();
                        if (step < shifts.size()) stepData.shift = shifts[step].getFloatValue();
                        if (step < swings.size()) stepData.swing = swings[step].getFloatValue();
                        if (step < pitches.size()) stepData.pitch = pitches[step].getIntValue();
                    }

                    for (int i = 0; i < juce::jmin(6, randoms.size()); ++i)
                        trackRandomAmounts[patternIndex][track][i].store(randoms[i].getFloatValue());
    ...
    {
        activePatternIndex.store(uiXml->getIntAttribute("activePatternIndex", 0));
        currentInstrument.store(uiXml->getIntAttribute("currentInstrument", 0));
        currentPage.store(uiXml->getIntAttribute("currentPage", 0));
        activeSection.store(uiXml->getIntAttribute("activeSection", -1));
        themeIndex.store(uiXml->getIntAttribute("themeIdx", 0));
        footerTabIndex.store(uiXml->getIntAttribute("footerTab", 0));
        uiScale.store(static_cast<float>(uiXml->getDoubleAttribute("uiScale", 1.0)));
        visibleTracks.store(uiXml->getIntAttribute("visibleTracks", 16));
    }
